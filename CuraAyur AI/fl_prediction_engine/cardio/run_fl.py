"""
run_fl.py — Run FL simulation + baseline comparison for Cardio.
...
def run_baseline():
    ...
    joblib.dump(model, "models/cardio_baseline_model.joblib")
    joblib.dump(scaler, "models/cardio_baseline_scaler.joblib")
    return metrics
...
def run_local_fl(num_rounds: int = 10, num_clients: int = 3):
    ...
    Path("models").mkdir(exist_ok=True)
    joblib.dump(global_model, "models/cardio_fl_model.joblib")
    joblib.dump(scaler, "models/cardio_scaler.joblib")
