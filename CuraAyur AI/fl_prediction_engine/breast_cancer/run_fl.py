"""
run_fl.py — Run FL simulation + baseline comparison for Breast Cancer.
...
def run_baseline():
    ...
    joblib.dump(model, "models/breast_baseline_model.joblib")
    joblib.dump(scaler, "models/breast_baseline_scaler.joblib")
    return metrics
...
def run_local_fl(num_rounds: int = 10, num_clients: int = 3):
    ...
    Path("models").mkdir(exist_ok=True)
    joblib.dump(global_model, "models/breast_fl_model.joblib")
    joblib.dump(scaler, "models/breast_scaler.joblib")
